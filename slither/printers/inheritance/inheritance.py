    ...
    def output(self, filename):
        ...
        for child in self.contracts:
            ...
            if child.inheritance:
                ...
                result['child_to_base'][child.name]['immediate'] = map(str, immediate)
                if not_immediate:
                    info += ", ["+ green(", ".join(map(str, not_immediate))) + "]"
                    result['child_to_base'][child.name]['not_immediate'] = map(str, not_immediate)
        ...
        for base in self.contracts:
            ...
            if children:
                ...
                result['base_to_child'][base.name]['immediate'] = map(str, immediate)
                if not_immediate:
                    info += ', [' + blue(", ".join(map(str, not_immediate))) + ']'
                    result['base_to_child'][base.name]['not_immediate'] = map(str, immediate)
        self.info(info)

        json = self.generate_json_result(info)
        self.add_dictionary_to_json(result, json)
